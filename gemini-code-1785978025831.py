...
def send_support_gift(user_id):
    ...
    try:
        bot.send_message(user_id, "Спасибо за тейк.")
        if art_url and phrase:
            bot.send_photo(user_id, art_url, caption=phrase)
        elif art_url:
            bot.send_photo(user_id, art_url)
        elif phrase:
...
def publish_entry(entry):
    ...
    if message_id:
        try:
            bot.copy_message(CHANNEL_ID, entry["user_id"], message_id)
        except Exception:
            logging.exception("Не удалось скопировать оригинал, публикую как обычный текст")
            bot.send_message(CHANNEL_ID, html.escape(entry["text"]))
    else:
        bot.send_message(CHANNEL_ID, html.escape(entry["text"]))

    bot.send_message(CHANNEL_ID, f"{signature}\n{tags_line}")
...
admin_custom_time_state = {} # admin_id -> index в очереди для ввода времени
...
# ---------- Интерактивная очередь (ОБНОВЛЕНО) ----------
def render_queue_markup():
    """Формирует список тейков в очереди с кнопками для каждого."""
    kb = types.InlineKeyboardMarkup()
    with SCHEDULE_LOCK:
        ...
        for idx, entry in enumerate(queue):
            ...
            lines.append(f"<b>#{idx + 1}</b> [{dt.strftime('%d.%m %H:%M')}] — <i>{html.escape(preview)}...</i>")
            
            # Кнопки для управления каждым конкретным тейком
            kb.row(
...
def cb_publish_now_review(call):
    """Публикует тейк прямо во время проверки модератором."""
    if not is_admin(call.from_user.id):
        bot.answer_callback_query(call.id, "Только для админов.")
        return
    review_id = int(call.data.split(":", 1)[1])
