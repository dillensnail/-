...
# Подпись тейка
SIGNATURE_QUOTE = '\n\n<blockquote>#тейк | @PsychoPromblem | takebot✧⁠*⁠。</blockquote>'
...
def publish_entry(entry):
    """Публикация от имени бота с полной поддержкой Premium Эмодзи"""
    ...
    if entry.get("media_group_messages"):
        msgs = bot.copy_messages(CHANNEL_ID, user_id, entry["media_group_messages"], caption=full_text, parse_mode="HTML")
        msg = msgs[0] if msgs else None
    else:
        # Для 100% сохранения всех кастомных/премиум эмодзи используем прямой перенос
        msg = bot.copy_message(CHANNEL_ID, user_id, msg_id, caption=full_text, parse_mode="HTML")
