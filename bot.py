...
# ==============================================================================
# 5. СИСТЕМА TW/CW (Отдельный фиксированный список)
# ==============================================================================
TW_CW_TAGS = [
    "Селфхарм",
...
def build_tw_cw_line(tw_cw_list: list) -> str:
    ...
    Порядок отображения строго соответствует порядку в TW_CW_TAGS, независимо от порядка выбора.
    """
    if not tw_cw_list:
        return ""
    
    # Сортировка по индексу в оригинальном списке TW_CW_TAGS
    sorted_tags = sorted(
...
CATEGORY_LABELS = {
    ...
    "диагнозы": "🧠 Диагнозы",
    "жизнь": "💬 Разное и интерактив",
}
CATEGORY_ORDER = ["симптомы", "нейроотличие", "диагнозы", "жизнь"]

# ==============================================================================
# 9. ТЕКСТ ПЛАШКИ ПОМОЩИ И РЕСУРСЫ
# ==============================================================================
HELP_RESOURCES = {
    "russia": [
    ...
    "ukraine": [
        ...
        "☎️ Lifeline Ukraine: 7333 — лінія підтримки психічного здоров’я.",
        "☎️ Лінія емоційної підтримки: 0 800 211 444",
        "☎️ Національна психологічна асоціація: 0 800 100 102",
        "☎️ Національна гаряча лінія: 116 123 або 0 800 500 335",
        "☎️ Урядова лінія: 1547 — допомога у випадках насильства, торгівлі людьми та психологічна підтримка."
    ],
    "belarus": [
        "☎️ Экстренная психологическая помощь: 133 — круглосуточно.",
        "☎️ Детская линия помощи: 8-801-100-16-11 — круглосуточно и анонимно.",
        "☎️ Телефоны доверия для взрослых: +375 17 352-44-44, +375 17 304-43-70",
        "☎️ Линия для детей, подростков и родителей: +375 17 263-03-03"
    ]
}
...
# ==============================================================================
# 10, 11. ОБЫЧНЫЕ ХЕШТЕГИ И СТРУКТУРА В КОДЕ
# ==============================================================================
MENTAL_HEALTH_TAGS = [
...
def load_tags() -> dict:
    ...
    with open(TAGS_FILE, "r", encoding="utf-8") as f:
        data = json.load(f)
        # Очистка от удалённых из обычных хештегов категорий
...
def cmd_addtag(message: types.Message) -> None:
    ...
    if len(parts) < 3 or parts[1].lower() not in CATEGORY_ALIASES:
        bot.reply_to(message, "Использование: /addtag симптомы|нейроотличие|диагнозы|жизнь название")
        return
...
def cmd_deltag(message: types.Message) -> None:
    ...
    parts = message.text.split(maxsplit=2)
    if len(parts) < 3 or parts[1].lower() not in CATEGORY_ALIASES:
        bot.reply_to(message, "Использование: /deltag симптомы|нейроотличие|диагнозы|жизнь название")
        return
    category = CATEGORY_ALIASES[parts[1].lower()]
    tag = parts[2].strip().lstrip("#").lower()
    with DATA_LOCK:
...
def cmd_start(message: types.Message) -> None:
    ...
    bot.send_message(
        ...
        "Оформлять тейк не надо, выберите хештеги и при необходимости TW/CW после отправки. "
...
def process_media_group(mg_id: str) -> None:
    ...
    user_drafts[user_id] = {
        ...
        "active_section": "tags", # 'tags' или 'twcw'
    }
...
# ==============================================================================
# 12. НАВИГАЦИЯ И КЛАВИАТУРА ВЫБОРА
# ==============================================================================
def build_picker_keyboard(draft: dict) -> types.InlineKeyboardMarkup:
    """Удобное меню с переключением между обычными тегами и TW/CW."""
    kb = types.InlineKeyboardMarkup(row_width=2)
    active_sec = draft.get("active_section", "tags")

    tw_count = len(draft.get("tw_cw", set()))
    tw_label = f"⚠️ TW/CW-предупреждения ({tw_count})" if tw_count > 0 else "⚠️ TW/CW-предупреждения"
    
    # Кнопки переключения режимов
    if active_sec == "tags":
        kb.row(types.InlineKeyboardButton("➡️ Перейти к TW/CW", callback_data=f"{CB_TOGGLE_SECTION}twcw"))
    else:
        kb.row(types.InlineKeyboardButton("⬅️ Вернуться к хештегам", callback_data=f"{CB_TOGGLE_SECTION}tags"))

    if active_sec == "tags":
        # Категории обычных хештегов
        ...
        # Обычные хештеги выбранных категорий
        tag_buttons = []
...
def send_category_keyboard(chat_id: int) -> None:
    ...
    bot.send_message(
        chat_id,
        f"Выберите хештеги и при необходимости TW/CW-предупреждения:{tw_summary}",
...
# ==============================================================================
# 8. ОБРАБОТКА ВЫБОРА TW/CW И ОТПРАВКА ПЛАШКИ ПОМОЩИ (СТРОГО 1 РАЗ)
# ==============================================================================
...
def cb_toggle_twcw(call: types.CallbackQuery) -> None:
    ...
    if 0 <= tw_idx < len(TW_CW_TAGS):
        ...
        # Автоматическая отправка плашки помощи РОВНО ОДИН РАЗ за сессию
