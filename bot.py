...
def send_support_gift(user_id):
    """Открытка сразу после отправки тейка на модерацию: 'Спасибо', затем арт + фраза поддержки."""
...
def publish_entry(entry):
    ...
    if message_id:
        try:
            # copy_message — публикация "от имени бота", без пометки "Переслано от",
            # сохраняет premium-эмодзи и форматирование оригинального сообщения.
