...
# ==============================================================================
# СИСТЕМА TW/CW (Отдельный фиксированный список)
# ==============================================================================
TW_CW_TAGS = [
    "Селфхарм",
...
def build_tw_cw_line(tw_cw_list: list) -> str:
    """
    Строка-предупреждение о чувствительном контенте для начала поста.
    Порядок отображения строго соответствует порядку в TW_CW_TAGS.
    """
    if not tw_cw_list:
        return ""
...
CATEGORY_ORDER = ["нейроотличие", "диагнозы", "симптомы", "жизнь"]

# ==============================================================================
# ТЕКСТ ПЛАШКИ ПОМОЩИ И РЕСУРСЫ
# ==============================================================================
HELP_RESOURCES_TEXT = (
    ...
    "☎️ Lifeline Ukraine:\n"
    "7333 — лінія підтримки психічного здоров’я.\n"
    "☎️ Лінія емоційної підтримки:\n"
    "0 800 211 444\n"
    "☎️ Національна психологічна асоціація:\n"
    "0 800 100 102\n"
    "☎️ Національна гаряча лінія:\n"
    "116 123 або 0 800 500 335\n"
    "☎️ Урядова лінія:\n"
    "1547 — допомога у випадках насильства, торгівлі людьми та психологічна підтримка.\n\n"
    "🇧🇾 <b>Беларусь</b>\n"
    "☎️ Экстренная психологическая помощь:\n"
    "133 — круглосуточно.\n"
    "☎️ Детская линия помощи:\n"
    "8-801-100-16-11 — круглосуточно и анонимно.\n"
    "☎️ Телефоны доверия для взрослых:\n"
    "+375 17 352-44-44\n"
    "+375 17 304-43-70\n"
    "☎️ Линия для детей, подростков и родителей:\n"
    "+375 17 263-03-03\n\n"
    "🤖 @DBT_Skills_Bot — кнопка SOS\n\n"
    "Обращаться за помощью не стыдно. Если ситуация прямо сейчас угрожает твоей жизни или безопасности — обратись в экстренные службы."
)
...
def get_channel_url() -> str:
    ...
    try:
        ...
        logging.warning("Не удалось получить ссылку на канал автоматически")
    return RULES_URL
...
# ==============================================================================
# ОБЫЧНЫЕ ХЕШТЕГИ И СТРУКТУРА
# ==============================================================================
MENTAL_HEALTH_TAGS = [
    "тревога", "паранойя", "срыв", "паническиеатаки", "диссоциация",
    "эмоциональнаярегуляция", "сенсорнаяперегрузка", "мелтдаун", "выгорание",
    "маскинг", "бессонница", "прокрастинация", "навязчивыемысли",
    "проблемыссамооценкой", "проблемысагрессией"
]

NEURODIVERGENT_TAGS = [
    "сдвг", "рас", "дислексия", "синдромтуретта", "диспраксия",
    "гиперфокус", "гиперфикс", "специнтерес"
]

DIAGNOSIS_TAGS = [
    "депрессия", "тревожноерасстройство", "бар", "шар", "прл",
    "птср", "кптср", "окр", "рпп", "дри", "шизофрения", "социофобия"
]

LIFE_TAGS = [
    "интерактив", "другое", "мемы", "полезное"
]
...
def load_tags() -> dict:
    ...
    with open(TAGS_FILE, "r", encoding="utf-8") as f:
        data = json.load(f)
        banned_from_regular = {"селфхарм", "суицидальныемысли", "ремиссия"}
        for cat in data:
            data[cat] = [t for t in data[cat] if t.lower() not in banned_from_regular]
        return data
...
def cmd_addtag(message: types.Message) -> None:
    ...
    if len(parts) < 3 or parts[1].lower() not in CATEGORY_ALIASES:
        bot.reply_to(message, "Использование: /addtag нейроотличие|диагнозы|симптомы|жизнь название")
        return
    category = CATEGORY_ALIASES[parts[1].lower()]
    tag = parts[2].strip().lstrip("#").lower()
    with DATA_LOCK:
        if tag in TAGS.get(category, []):
...
def cmd_deltag(message: types.Message) -> None:
    ...
    parts = message.text.split(maxsplit=2)
    if len(parts) < 3 or parts[1].lower() not in CATEGORY_ALIASES:
        bot.reply_to(message, "Использование: /deltag нейроотличие|диагнозы|симптомы|жизнь название")
        return
    category = CATEGORY_ALIASES[parts[1].lower()]
    tag = parts[2].strip().lstrip("#").lower()
    with DATA_LOCK:
...
def cmd_start(message: types.Message) -> None:
    ...
    bot.send_message(
        ...
        "Сначала выберите категорию, затем TW/CW и подтеги. Публикация полностью анонимна!",
        reply_markup=kb,
    )
...
def process_media_group(mg_id: str) -> None:
    ...
    user_drafts[user_id] = {
        ...
        "active_section": "category",  # Строго начинаем с первого шага — выбора категории!
    }
...
# ==============================================================================
# ПОШАГОВАЯ НАВИГАЦИЯ И КЛАВИАТУРА ВЫБОРА (1. Категория -> 2. TW/CW -> 3. Подтеги/Текст)
# ==============================================================================
def get_step_text(draft: dict) -> str:
    sec = draft.get("active_section", "category")
    if sec == "category":
        return "<b>Шаг 1 из 3.</b> Выберите категорию вашего тейка:"
    elif sec == "twcw":
        cat_names = ", ".join(CATEGORY_LABELS.get(c, c) for c in draft["categories"])
        tw_summary = ""
        if draft.get("tw_cw"):
            tw_summary = f"\n\n<b>Выбранные TW/CW:</b> {', '.join(draft['tw_cw'])}"
        return f"<b>Шаг 2 из 3.</b> Выберите TW/CW-предупреждения (если необходимы):\n<i>Категория: {cat_names}</i>{tw_summary}"
    else:
        tw_summary = ""
        if draft.get("tw_cw"):
            tw_summary = f"\n<b>TW/CW:</b> {', '.join(draft['tw_cw'])}"
        return f"<b>Шаг 3 из 3.</b> Выберите подтеги и подтвердите отправку:{tw_summary}"

def build_picker_keyboard(draft: dict) -> types.InlineKeyboardMarkup:
    kb = types.InlineKeyboardMarkup(row_width=2)
    active_sec = draft.get("active_section", "category")

    if active_sec == "category":
        # ШАГ 1: ТОЛЬКО выбор категории
        ...
        # Переход к TW/CW доступен ТОЛЬКО после успешного выбора хотя бы одной категории
        if draft["categories"]:
            kb.row(types.InlineKeyboardButton("➡️ Далее к TW/CW", callback_data=f"{CB_TOGGLE_SECTION}twcw"))
        kb.row(types.InlineKeyboardButton("❌ Отменить", callback_data=CB_CANCEL))

    elif active_sec == "twcw":
        # ШАГ 2: Отдельная плашка TW/CW
        for idx, tw_item in enumerate(TW_CW_TAGS):
            is_selected = tw_item in draft.get("tw_cw", set())
            prefix = "☑️ " if is_selected else "🔲 "
            kb.row(types.InlineKeyboardButton(f"{prefix}{tw_item}", callback_data=f"{CB_PREFIX_TWCW}{idx}"))
        
        kb.row(types.InlineKeyboardButton("⬅️ Назад к категориям", callback_data=f"{CB_TOGGLE_SECTION}category"))
        kb.row(types.InlineKeyboardButton("➡️ Далее к подтегам и завершению", callback_data=f"{CB_TOGGLE_SECTION}tags"))
        kb.row(types.InlineKeyboardButton("❌ Отменить", callback_data=CB_CANCEL))

    else:
        # ШАГ 3: Дополнительные подтеги и отправка
        tag_buttons = []
        ...
        kb.row(types.InlineKeyboardButton("⬅️ Назад к TW/CW", callback_data=f"{CB_TOGGLE_SECTION}twcw"))
        kb.row(
...
def send_category_keyboard(chat_id: int) -> None:
    ...
    draft["active_section"] = "category"  # Убеждаемся, что всегда старт с 1 шага
    bot.send_message(
        chat_id,
        get_step_text(draft),
...
@bot.callback_query_handler(func=lambda c: c.data.startswith(CB_TOGGLE_SECTION))
def cb_toggle_section(call: types.CallbackQuery) -> None:
    ...
    # СТРОГАЯ ПРОВЕРКА: запрет показа TW/CW или подтегов без выбранной категории
    if sec in ["twcw", "tags"] and not draft["categories"]:
        bot.answer_callback_query(call.id, "❌ Сначала необходимо выбрать хотя бы одну категорию!", show_alert=True)
        return

    draft["active_section"] = sec
...
def cb_toggle_twcw(call: types.CallbackQuery) -> None:
    ...
    tw_idx = int(call.data.split(":", 1)[1])
    if 0 <= tw_idx < len(TW_CW_TAGS):
        tw_item = TW_CW_TAGS[tw_idx]
        tw_set = draft.setdefault("tw_cw", set())
        
        is_new_addition = tw_item not in tw_set
        if is_new_addition:
            tw_set.add(tw_item)
        else:
            tw_set.remove(tw_item)

        # Отправка плашки помощи РОВНО ОДИН РАЗ при выборе чувствительной темы
        if is_new_addition and not draft.get("help_message_sent"):
