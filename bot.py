...
def send_support_gift(user_id):
    """Отправляет благодарность и открытку ОДНИМ сообщением."""
...
def get_post_url(msg):
    """Генерирует прямую ссылку на опубликованный пост в канале."""
...
def publish_entry(entry):
    ...
    if not photos:
        ...
        msg = bot.send_photo(CHANNEL_ID, photos[0], caption=caption_text, parse_mode="HTML")
    else:
        media = [types.InputMediaPhoto(photos[0], caption=caption_text, parse_mode="HTML")]
        for p in photos[1:]:
    ...
    try:
        ...
        logging.exception("Не удалось отправить уведомление автору")
...
# Буфер для сборки альбомов (нескольких фото)
...
def cmd_start(message):
    ...
    bot.send_message(
        ...
        "Премиум эмоджи и фото поддерживаются",
        reply_markup=kb,
    )
...
def render_queue_markup():
    ...
    with SCHEDULE_LOCK:
        ...
        for idx, entry in enumerate(queue):
            ...
            if not preview_text and entry.get("photos"):
                preview_text = f"[📷 Фото: {len(entry['photos'])} шт]"
            elif entry.get("photos"):
...
def cb_queue_view(call):
    ...
    media_info = f"\n🖼 <b>Прикреплено фото:</b> {photos_count}" if photos_count else ""
    
    view_text = (
        f"📄 <b>Тейк #{idx + 1} в очереди</b>\n"
        f"⏰ <b>Время:</b> {dt} (мск){media_info}\n\n"
...
# ---------- Команды админов для открыток/фраз ----------
...
# ---------- Приём тейка (текст, одиночные фото или альбомы) ----------
def process_media_group(mg_id):
    """Финализирует приём альбома фото от пользователя."""
...
def handle_incoming_take(message):
    ...
    # Если отправлен альбом (медиагруппа)
    ...
    # Если одиночное фото или текст
    photos = []
    text_html = ""
    if message.photo:
        photos.append(message.photo[-1].file_id)
        text_html = message.html_caption or ""
    else:
        text_html = message.html_text or ""
...
def cb_confirm(call):
    ...
    # Отправка открытки единым сообщением
    ...
    if not photos:
        ...
        admin_msg = bot.send_photo(ADMIN_CHAT_ID, photos[0], caption=admin_text, reply_markup=kb, parse_mode="HTML")
    else:
...
def cb_publish_now_review(call):
    ...
    try:
        publish_entry(item)
        bot.edit_message_text(f"⚡️ Опубликовано мгновенно ({admin_name}).",
                               call.message.chat.id, call.message.message_id,
                               reply_markup=types.InlineKeyboardMarkup())
        bot.answer_callback_query(call.id, "Опубликовано!")
    except Exception:
