...
# Исправленная подпись с рабочей ссылкой на бота
SIGNATURE_QUOTE = '\n\n<blockquote>#тейк | @PsychoPromblem | <a href="https://t.me/PsychoPromblembot">takebot✧⁠*⁠。</a></blockquote>'
...
def publish_entry(entry):
    ...
    if entry.get("media_group_messages"):
        ...
        # caption не применяется к обычному тексту — подпись отдельным сообщением
        stored_msg = bot.copy_message(STORAGE_CHANNEL_ID, user_id, msg_id)
...
def cb_confirm(call):
    ...
    if item.get("media_group_messages"):
        ...
        # caption не работает для обычного текста — копируем сообщение, а заголовок с тегами шлём отдельно
        bot.copy_message(ADMIN_CHAT_ID, user_id, item["message_id"])
        admin_msg = bot.send_message(ADMIN_CHAT_ID, admin_text, reply_markup=kb, parse_mode="HTML")
    else:
