...
# Подпись тейка
SIGNATURE_QUOTE = '<blockquote>#тейк | @PsychoPromblem | takebot✧⁠*⁠。</blockquote>'
...
def publish_entry(entry):
    """Публикация от лица бота (полная анонимность)"""
...
# ---------- Команда просмтора очереди для админов ----------
...
def cb_q_del(call):
    ...
    with SCHEDULE_LOCK:
        if 0 <= idx < len(SCHEDULE["queue"]):
            ...
            save_schedule(SCHEDULE)
            bot.answer_callback_query(call.id, f"Тейк #{idx+1} удалён из очереди!")
...
def handle_edited_take(message):
    ...
    # Проверяем, есть ли текущие заявки от пользователя
...
def update_admin_review_card(review_id):
    """Обновление карточки в админ-чате при авто-редактировании сообщения юзером"""
