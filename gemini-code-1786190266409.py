...
def send_help_resources(chat_id: int) -> None:
    ...
    # Кнопки с поддержкой вызова/копирования номеров
...
def build_wizard_keyboard(draft: dict) -> types.InlineKeyboardMarkup:
    ...
    # ШАГ 2: Выберите категорию (ТОЛЬКО для "Отправить тейк")
    ...
    # ШАГ 2.1: Выбор хештегов категории (ПО 2 КНОПКИ В РЯД)
    ...
    # ШАГ 3: Добавить симптомы/состояния?
    ...
    # ШАГ 3.1: Выбор симптомов (по 2 в ряд)
    ...
    # Выбор доп. хештегов для упрощенной ветки
    ...
    # ПРЕДПРОСМОТР (Раздельные клавиатуры)
    elif step == "preview":
        kb.row(types.InlineKeyboardButton("✅ Отправить тейк", callback_data=CB_CONFIRM))
        
        # Для категорий 2-5 (Интерактив, Мем, Полезное, Другое)
        if draft.get("pub_type") in QUICK_PUB_TAGS:
            ...
            # Только для кнопки "Отправить тейк"
...
def cb_select_pub_type(call: types.CallbackQuery) -> None:
    ...
    # ВЕТКА 2-5: Интерактив, Мем, Полезное, Другое
    if pub_type in QUICK_PUB_TAGS:
        tag_name = QUICK_PUB_TAGS[pub_type]
        draft["tags"].add(tag_name)
        draft["step"] = "preview"  # Сразу переход к предпросмотру
    else:
        # ВЕТКА 1: Отправить тейк
...
def cb_select_category(call: types.CallbackQuery) -> None:
    ...
    # После выбора категории автоматически переходим к выбору хештегов по 2 в ряд
    ...
    if 0 <= idx < len(TW_CW_TAGS):
        ...
        if item in draft["tw_cw"]:
            ...
            if not draft.get("help_message_sent"):
                ...
                send_help_resources(user_id)  # Вызов экстренного меню с кнопками номеров
