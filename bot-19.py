...
CRISIS_NOTE = (
    ...
    "☎️ 8-800-200-31-00 или 7 (499) 288-31-00 — экстренная психологическая помощь, Россия, 7:00–23:00 мск, бесплатно\n"
    "🤖 Бот @DBT_Skills_Bot — кнопка SOS\n\n"
    'Полная подборка горячих линий и бесплатных консультаций (в т.ч. Украина, Беларусь): '
    '<a href="https://t.me/ADHDKF/9634">пост с ресурсами</a>\n\n'
...
def publish_entry(entry):
    ...
    """
    all_tags = get_entry_categories(entry) + entry["tags"]
...
def build_category_keyboard(selected_categories):
    ...
    for key in CATEGORY_ORDER:
        ...
        kb.add(types.InlineKeyboardButton(f"{prefix}{CATEGORY_LABELS[key]}", callback_data=f"cat:{key}"))
    kb.row(types.InlineKeyboardButton("Далее ➜", callback_data="cat_next"))
    kb.row(types.InlineKeyboardButton("❌ Отменить", callback_data="cancel"))
    return kb

def send_category_keyboard(chat_id):
    bot.send_message(
        chat_id,
        "Выберите категории (можно несколько — например, и «Расстройства», и «Нейроотличия»):",
        reply_markup=build_category_keyboard(set())
    )
...
def build_subtag_keyboard(user_id):
    ...
    kb.add(*buttons)
    kb.row(types.InlineKeyboardButton("◀️ Изменить категории", callback_data="backcat"))
    kb.row(
...
# ---------- Выбор категорий (мультивыбор) и тегов ----------
...
def cb_choose_category(call):
    ...
    bot.edit_message_text(
        ...
        reply_markup=build_category_keyboard(draft["categories"])
    )
    bot.answer_callback_query(call.id)

@bot.callback_query_handler(func=lambda c: c.data == "cat_next")
def cb_category_next(call):
    user_id = call.from_user.id
    draft = user_drafts.get(user_id)
    if not draft or not draft["categories"]:
        bot.answer_callback_query(call.id, "Выберите хотя бы одну категорию.")
        return
    bot.edit_message_text(
        "Выберите теги:",
        call.message.chat.id, call.message.message_id,
...
def cb_back_to_category(call):
    user_id = call.from_user.id
    draft = user_drafts.get(user_id)
    selected = draft["categories"] if draft else set()
    bot.edit_message_text(
        "Выберите категории (можно несколько):",
        call.message.chat.id, call.message.message_id,
...
def cb_toggle_tag(call):
    ...
    bot.answer_callback_query(call.id)
    if just_added and tag in SENSITIVE_TAGS:
        try:
...
def update_admin_review_card(review_id):
    ...
    all_tags = get_entry_categories(item) + item["tags"]
    tags_line = "\n" + " ".join(f"#{t}" for t in all_tags)
