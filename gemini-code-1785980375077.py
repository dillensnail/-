...
def publish_entry(entry):
    ...
    if FORWARD_MODE:
        if entry.get("media_group_messages"):
            for m_id in entry["media_group_messages"]:
                bot.forward_message(CHANNEL_ID, user_id, m_id)
        else:
            bot.forward_message(CHANNEL_ID, user_id, msg_id)

        caption_text = f"{signature_quote}\n{tags_line}"
        msg = bot.send_message(CHANNEL_ID, caption_text, parse_mode="HTML")
    else:
        original_text = entry.get("text_html", "").strip()
        full_caption = f"{original_text}\n\n{signature_quote}\n{tags_line}" if original_text else f"{signature_quote}\n{tags_line}"

        if entry.get("media_group_messages"):
            msgs = bot.copy_messages(CHANNEL_ID, user_id, entry["media_group_messages"], caption=full_caption, parse_mode="HTML")
            msg = msgs[0] if msgs else None
        else:
            msg = bot.copy_message(CHANNEL_ID, user_id, msg_id, caption=full_caption, parse_mode="HTML")
...
def cmd_start(message):
    ...
    forward_status = "⭐ Пересыл от бота: <b>Включён 🟢</b>" if FORWARD_MODE else "⚙️ Копирование: Включено"
    
    bot.send_message(
        message.chat.id,
        f"🌸 Отправьте свой тейк админам\n\n"
        f"{forward_status}\n\n"
...
@bot.callback_query_handler(func=lambda c: c.data.startswith("user_edit:"))
def cb_user_edit(call):
    review_id = int(call.data.split(":")[1])
    if review_id not in pending_review:
        bot.answer_callback_query(call.id, "Тейк уже обработан или опубликован!", show_alert=True)
        return

    user_editing_state[call.from_user.id] = review_id
    bot.send_message(
        call.message.chat.id,
        "✏️ Отправьте новый текст или медиа для вашего тейка:"
    )
...
def process_media_group(mg_id):
    ...
    if user_id in user_editing_state:
        review_id = user_editing_state.pop(user_id)
        if review_id in pending_review:
            pending_review[review_id]["text_html"] = data["caption_html"]
            pending_review[review_id]["message_id"] = data["messages"][0]
            pending_review[review_id]["media_group_messages"] = data["messages"]
            update_admin_review_card(review_id)
            bot.send_message(user_id, "✅ Ваш тейк обновлён!")
        return
...
def handle_incoming_take(message):
    ...
    if user_id in user_editing_state:
        ...
        if review_id in pending_review:
            ...
            pending_review[review_id]["text_html"] = text_html
            pending_review[review_id]["message_id"] = message.message_id
            pending_review[review_id]["media_group_messages"] = None
            update_admin_review_card(review_id)
            bot.send_message(user_id, "✅ Ваш тейк обновлён у модераторов!")
        return
...
def update_admin_review_card(review_id):
    ...
    admin_text = f"📝 <b>Тейк на проверке (#{review_id}) [ОБНОВЛЁН АВТОРОМ]</b>\n\n{preview_body}"
...
def cb_confirm(call):
    ...
    user_control_kb.row(
        types.InlineKeyboardButton("✏️ Редактировать", callback_data=f"user_edit:{review_id}"),
...
def cb_ban_user(call):
    ...
    # Резервный вариант, если тейк отменили, но ID остался
    ...
    chat_status = "и выбит из чата 🚪" if kicked_from_chat else "(не был в чате / нет прав на бан)"
    
    bot.edit_message_reply_markup(call.message.chat.id, call.message.message_id, reply_markup=types.InlineKeyboardMarkup())
    bot.send_message(
        call.message.chat.id,
        f"🚫 <b>Пользователь {target_user_id} забанен в боте {chat_status}</b> ({admin_name}).",
